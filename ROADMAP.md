...
Thirteen milestones, about 81 person weeks at 25 focused hours a week. Each milestone has an issue with its exit criteria on it, and the issue is the thing that gets updated. This file is the map.
...
|---|---|---|---|
| M0 | Prove the environment | 5 | The build pipeline, the toolchain tarball, the pass plugin magic, Alive2 wired up, the Pass Tape, and two pilot lessons |
| M1 | Part 0 and the Tourist | 7 | Sixteen lessons, the site, the claim ledger. **Stopping point.** |
| M2 | The IR | 6 | Ten lessons, ten Blueprints, and `bpc` generating attributes and intrinsics from TableGen |
| M3 | Analyses and the pass manager | 5 | Nine lessons, eight Blueprints, the invalidation grader |
| M4 | Transforms | 8 | Fourteen lessons, ten pass Blueprints, and the Alive2 CI job running every refinement obligation. **Stopping point.** |
| M5 | Correctness | 4 | Six lessons on the verifier, reduction, lit and Alive2 |
| M6 | Version bump rehearsal | 4 | No new lessons. Bump to LLVM 24 on purpose and measure what it cost |
| M7 | SelectionDAG | 6 | Nine lessons, three codegen Blueprints, the DAG viewer |
| M8 | MIR and below | 7 | Ten lessons, five Blueprints, register allocation and the MC layer |
| M9 | GlobalISel and TableGen | 7 | Fourteen lessons, and `gen_td` generating target Blueprints from `-dump-json` |
| M10 | The rest of the toolchain | 5 | Eight lessons: Clang, MLIR, lld, LTO, ORC, sanitizers, PGO |
| M11 | The Surgeon | 5 | Six in tree modification lessons, ending in a real pull request. **Stopping point.** |
| M12 | Capstones | 12 | All three tracks with skeletons, graders, scorecards and reference solutions |
